(() => {
    ...
    runtimeStyle.textContent = `
        ...
        body { overflow-x: hidden; overflow-y: auto !important; cursor: default !important; }
        a, button, [role="button"] { cursor: pointer !important; }
        .page-loader, .custom-cursor, .cursor-trail { display: none !important; }
        ...
        .reveal.visible { opacity: 1; visibility: visible; transform: none; filter: blur(0); }

        .navbar { position: fixed !important; top: 0 !important; z-index: 5000 !important; transition: box-shadow .3s ease, background .3s ease; }
        .navbar.scrolled { background: rgba(255,255,255,.9) !important; box-shadow: 0 12px 38px rgba(16,24,40,.09); }

        .video-wrapper { position: relative; overflow: hidden; }
        .video-thumbnail-link { position: absolute; inset: 0; display: block; overflow: hidden; background: #08111f; cursor: pointer !important; }
        .video-thumbnail-link img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; transition: transform .65s cubic-bezier(.16,1,.3,1), filter .45s ease; }
        .video-thumbnail-link::after { content: ""; position: absolute; inset: 0; background: linear-gradient(180deg, rgba(0,0,0,.02), rgba(0,0,0,.12) 45%, rgba(0,0,0,.65)); }
        .video-thumbnail-link:hover img { transform: scale(1.07); filter: saturate(1.1) contrast(1.04); }
        .video-play { position: absolute; z-index: 2; left: 50%; top: 50%; width: 72px; height: 72px; display: grid; place-items: center; border: 1px solid rgba(255,255,255,.55); border-radius: 50%; background: rgba(255,255,255,.15); backdrop-filter: blur(12px); transform: translate(-50%,-50%); transition: transform .3s ease, background .3s ease; }
        .video-play span { width: 0; height: 0; margin-left: 5px; border-top: 9px solid transparent; border-bottom: 9px solid transparent; border-left: 14px solid white; }
        .video-thumbnail-link:hover .video-play { transform: translate(-50%,-50%) scale(1.12); background: rgba(22,119,255,.82); }

        .review-card, .collab-card, .video-card { transform-style: preserve-3d; will-change: transform; transition: transform .22s ease, box-shadow .35s ease; }
